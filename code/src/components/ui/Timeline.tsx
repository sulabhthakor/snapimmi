...
export function Timeline({ events, emptyMessage = "No activity yet." }: TimelineProps) {
    ...
    return (
        <div className="space-y-8 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-primary-teal-300 before:to-transparent">
            {events.map((event, i) => (
                <div key={i} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                    ...
                    {/* Card */}
                    <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-white/80 backdrop-blur-sm p-4 rounded-xl border border-gray-200/50 shadow-sm hover:shadow-[0_4px_12px_0_rgba(44,129,141,0.1)] transition-all">
                        <div className="flex items-center justify-between space-x-2 mb-1">
                            <div className="font-bold text-gray-900 text-sm">{event.title}</div>
                            <time className="font-medium text-primary-teal-600 text-xs">{event.date.toLocaleDateString()}</time>
                        </div>
                        {event.subtitle && (
